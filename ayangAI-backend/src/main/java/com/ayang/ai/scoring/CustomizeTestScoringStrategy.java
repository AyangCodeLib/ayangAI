...
public class CustomizeTestScoringStrategy implements ScoringStrategy {
    ...
    public UserAnswer doScore(List<String> choices, App app) throws Exception {
        ...
        for (ScoringResult scoringResult : scoringResultList) {
            ...
            int score = resultProp.stream()
                    .mapToInt(prop -> optionCount.getOrDefault(prop, 0) + 1)
                    .sum();

            // 如果分数高于当前最高分数，更新最高分数和最高分数对应的测评结果
            if (score > maxScore) {
                maxScore = score;
